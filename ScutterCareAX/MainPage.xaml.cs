...
namespace MazikCare.MobEval
{
    ...
    /// </summary>
    public sealed partial class MainPage : MazikCare.MobEval.Common.LayoutAwarePage
    {
        XmlDocument tileXml;
        ...
        private async void pageRoot_Loaded_1(object sender, RoutedEventArgs e)
        {
            ...
            if (app.SettingsData.IsSyncWithAX)
            {
                ...
                this.lblPatient1.Text = prosp[0].parmName;
                try
                {
                    this.imgPatient1.Source = await srv.GetPatientImage(prosp[0].parmRecId);
                }
                catch (Exception)
                {
                }
                ...
                this.lblPatient2.Text = prosp[1].parmName;
                try
                {
                    this.imgPatient2.Source = await srv.GetPatientImage(prosp[1].parmRecId);
                }
                catch (Exception)
                {
                }

                this.lblPatient3.Text = prosp[2].parmName;
                try
                {
                    this.imgPatient3.Source = await srv.GetPatientImage(prosp[2].parmRecId);
                }
                catch (Exception)
                {
                }

                this.lblPatient4.Text = prosp[3].parmName;
                try
                {
                    this.imgPatient4.Source = await srv.GetPatientImage(prosp[3].parmRecId);
                }
                catch (Exception)
                {
                }
                this.lblPatient5.Text = prosp[4].parmName;
                try
                {
                    this.imgPatient5.Source = await srv.GetPatientImage(prosp[4].parmRecId);
                }
                catch (Exception)
                {
                }
            ...
            if (app.SettingsData != null && app.Patient != null)
                if (app.SettingsData.IsSyncWithAX)
                {
